    ...
    <footer className="mt-24 bg-ink text-background">
      <div className="container-prose py-16 grid gap-12 lg:grid-cols-5">
        <div className="lg:col-span-2">
          ...
          <div className="flex gap-3 mt-5">
            {[Facebook, Instagram, Twitter, Linkedin, Youtube].map((Icon, i) => (
              <a key={i} href="#" className="w-9 h-9 rounded-full border border-background/20 flex items-center justify-center hover:bg-royal hover:text-white hover:border-royal transition-colors">
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm text-background/70">
            {[["/financial-literacy","Financial Literacy"],["/entrepreneurship","Entrepreneurship"],["/leadership-development","Leadership"],["/programs-activities","Programs & Activities"],["/events","Events"]].map(([to,l])=>(
              <li key={to}><Link to={to} className="hover:text-royal">{l}</Link></li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm text-background/70">
            {[["/join-future-founders","Join Future Founders"],["/sponsors","Become a sponsor"],["/donate","Donate"],["/contact","Contact us"],["/faqs","FAQs"]].map(([to,l])=>(
              <li key={to}><Link to={to} className="hover:text-royal">{l}</Link></li>
            ))}
          </ul>
